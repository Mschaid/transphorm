...
class BranchingCNNDecoder(nn.Module):
    def __init__(self, output_channels=3, base_dim=16):
        ...
        self.long_branch = nn.Sequential(
            nn.ConvTranspose1d(
                base_dim * 2, base_dim, kernel_size=27, padding=1, dilation=16
            ),
            nn.ReLU(),
            nn.ConvTranspose1d(
                base_dim, output_channels, kernel_size=27, padding=1, dilation=16
            ),
            nn.ReLU(),
        )
        self.mid_branch = nn.Sequential(
            nn.ConvTranspose1d(
                base_dim * 2, base_dim, kernel_size=9, padding=1, dilation=8
            ),
            nn.ReLU(),
            nn.ConvTranspose1d(
                base_dim, output_channels, kernel_size=9, padding=1, dilation=8
            ),
            nn.ReLU(),
        )
        self.local_branch = nn.Sequential(
