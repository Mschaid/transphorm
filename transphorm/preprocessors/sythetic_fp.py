...
class SytheticTSDataMoudle(L.LightningDataModule):
    ...
    def create_square_wave(self, freq):
        ...
        """
        num_points = 1000
        random_multiplier = np.random.uniform(0, 2)
    ...
    def create_dataset(self):
        ...
        for i in range(int((self.sample_size / 2))):
            random_multiplier = np.random.uniform(0, 3)
            random_freq = np.random.uniform(1, 100)
            sines.append(self.create_sinewave(random_freq))
            squares.append(create_square_wave(random_freq))
        X = np.vstack((sines, squares))

        sine_lables = np.zeros(int((self.sample_siz / 2)))
